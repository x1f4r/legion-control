...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, MenuBarHost {
    ...
    private static let isolatedFrameKey = "windowFrame"
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Tell the update helper this build came up. Without the marker it puts the previous bundle
    ...
    private func makeWindow() {
        ...
        if AppPaths.homeOverride != nil {
            if let saved = AppPreferences.string(forKey: Self.isolatedFrameKey) {
                window.setFrame(NSRectFromString(saved), display: false)
            } else {
                window.center()
            }
        } else {
            let key = "NSWindow Frame \(Self.frameAutosaveName)"
    ...
    private func rememberIsolatedFrame() {
        guard AppPaths.homeOverride != nil, let window else { return }
        AppPreferences.set(NSStringFromRect(window.frame), forKey: Self.isolatedFrameKey)
    }
}
