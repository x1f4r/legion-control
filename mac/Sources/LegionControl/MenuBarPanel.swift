        ...
        PanelSection(title: title) {
            ...
            HStack(spacing: 8) {
                PrimaryActionButton(
                    ...
                    .help(service.updateUnavailableReason ?? "Installs the newer build and starts it again.")
    ...
    var body: some View {
        ...
        if let failure = mac.failure {
            ...
            ForEach(Array(mac.services.enumerated()), id: \.element.id) { index, service in
                block(service, showsToggle: index == 0)
                PanelRule()
            }
        }
    }
            ...
            HStack(alignment: .center, spacing: 14) {
                PrimaryActionButton(
                    ...
                    .help(mac.updateUnavailableReason(service)
                        ?? "Quits \(service.displayName) on this Mac and starts it again on the new build. Held back while work is running.")
                ...
                if showsToggle {
                    ...
                    .help("Apply a waiting build without asking, once nothing is running.")
                }
            }
            .padding(.top, 2)
